...
public class JDBC_Search_Test extends JFrame
{
    ...
    public JDBC_Search_Test()
    {
        ...
        searchBtn.addActionListener(new ActionListener() {
            ...
            {
                String nameToSearch = customerNameField.getText().trim();
                String cityToSearch = cityField.getText().trim();
                String countryToSearch = countryField.getText().trim();
                int minCreditToSearch = 0; // may have to convert to int
                int maxCreditToSearch = Integer.MAX_VALUE;
                
                try
                {
                    if(!minCreditLimitField.getText().trim().isEmpty())
                        {
                            minCreditToSearch = Integer.parseInt(minCreditLimitField.getText().trim());
                        }
                    
                    if(!maxCreditLimitField.getText().trim().isEmpty())
                        {
                            maxCreditToSearch = Integer.parseInt(maxCreditLimitField.getText().trim());
                        }
                }
                
                catch (NumberFormatException nfe)
                {
                    JOptionPane.showMessageDialog(frame, "Please eneter valid numeric values for the minimum and maximum credit limit.");
                    return;
                }
    ...
    public void SearchAndDisplayResults (String nameToSearch, String cityToSearch, String countryToSearch, int minCreditToSearch, int maxCreditToSearch)
    {
        ...
        try 
        {
            ...
            // create SQL query
          //  String sql = "SELECT customerNumber, customerName, city, country, creditLimit FROM customers WHERE country IN ('" + countryToSearch+ "') ORDER BY customerNumber";
            String sql = "SELECT customerNumber, customerName, city, country, creditLimit FROM customers WHERE 1=1";

            // modify SQL query depending on what search fields are used
            if (!nameToSearch.isEmpty())
            {
                sql += " AND customerName LIKE '%" + nameToSearch + "%'";
            }
            
            if (!cityToSearch.isEmpty())
            {
                sql += " AND city LIKE '%" + cityToSearch + "%'";
            }
            
            if (!countryToSearch.isEmpty())
            {
                sql += " AND country LIKE '%" + countryToSearch + "%'";
            }
            
            sql += " AND creditLimit BETWEEN " + minCreditToSearch + " AND " + maxCreditToSearch;
            sql += " ORDER BY customerNumber";
    ...
    public static void main(String[] args) 
    {
        ...
        // Data to be displayed in the JTable object
        Object[][] data = 
        {
            {1, "Alice B. Toklas", "0431 888 555",
            "alicebtoklas@gmail.com", true},
            {2, "Gertrude Stein", "0400 777 666",
            "gertrudestein@yahoo.com", false}
        };
        
        // Column Names
        String[] columnNames = {"ID", "Name", "Mobile", "Email", "Status"};
        
        // Initialise the JTable object
        JTable friendsTable = new JTable(data, columnNames);
        
        // set position in the frame and size limits
        // --- setBounds (x, y, width, height)
        friendsTable.setBounds(30, 40, 200, 300);
        
        // add the JTable object to a JScrollPane object
        JScrollPane sp = new JScrollPane(friendsTable);
        
        // add JScrollPane object to JFrame
        JFrame frame = new JFrame();
        frame.add(sp);
        frame.setTitle("Friends Table Example");
